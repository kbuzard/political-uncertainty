...
\noindent This document presents the summary for the summer of 2020 work on the political uncertainty project. More detail is in ``Road\_Map.txt.''
\begin{enumerate}
...
\begin{itemize}
	\item ``Error in file(con, ``w") : cannot open the connection
	\item In addition: Warning message:
In file(con, ``w") :
  cannot open file \url{C:\Users\javaldeb\AppData\Local\Temp\Rtmp2veS6l\file4dbc4163331}: No such file or directory"

\end{itemize}
Troubleshooting this I found different solutions, but non have worked yet. The most likely explanation is that I do not have write access. The literature recommended setting the working directory again, but it didn't fixed the problem. Another proposed solution was to restart RStudio and reinstall the packages. I tried this and it also didn't work. 


After checking the documentation for STAN command, I found no mistakes in the code, particularly line 37 (I meant it is not commented out, its just the syntax) and its generating a vector of values that it is called back in line 105. Which is where I get the error described before. 
...
One detail that creates confusion is that Yusuf's and Yimin's work are located in the same folders. 
...
The variables on this data set are:
...
The website voteview.com also has the following datasets:
\begin{itemize}
...
Both of these	 data sets are merged by action\_id.

The API key I requested is: 8d6d8a79f2f3df97ef128dbfea969a82 and the link is:  \url{https://maplight.org/data\_guide/bulk-data-sets-and-apis/}
...
\begin{itemize}
	\item The ``destring" command made the mistake of deleting the prefix of bills (H, HR, ...) therefore, it was treating different bills as if they were one. 
	\item Example: bill numbers H2 and HR2. The ``destring" command meant that both bills were listed as number 2, treating them as the same for merging with the votes.dta data set. 
	\item This problem is fixed with RSTAN, given that it is not using ``destring".
...
\begin{itemize}
	\item Generate\_vote\_data\_for\_R.do
	\item Generate\_group\_dummy\_by\_action.do
\end{itemize}

Both are in the github rep and the G disk inside the folder:
\url{political-uncertainty/build/code/}

The RStan\_model.R is also located in the Github repo and on the G drive in the folder:
